...
#@ (os itens da primeira coluna vão para a primeira linha e assim por diante)
def transposta(matriz):
    matrix = cria(len(matriz[0]),len(matriz))
    for coluna in range(len(matriz[0])):
        for linha in range(len(matriz)):
            matrix[coluna][linha] = matriz[linha][coluna]
    return matrix